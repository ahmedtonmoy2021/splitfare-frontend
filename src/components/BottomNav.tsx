...
type Key = 'home' | 'search' | 'trips' | 'requests' | 'profile';
  ...
  return (
    <View style={[styles.bar, { paddingBottom: insets.bottom + 10 }]}>
      {ITEMS.map((item) => {
        ...
        const color = on ? colors.blue : '#9aa8bb';
        return (
          <TouchableOpacity
            key={item.key}
            style={styles.item}
...
const styles = StyleSheet.create({
  bar: { flexDirection: 'row', justifyContent: 'space-between', backgroundColor: colors.white, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 12, paddingHorizontal: 22 },
